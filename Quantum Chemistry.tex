% FIXME: Cannot import figures on Linux platform
\documentclass{book}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{geometry}
\usepackage{graphicx}
\usepackage[colorlinks=true]{hyperref}
\usepackage{fancyhdr}
\usepackage{booktabs}
\usepackage{array}
\usepackage{multirow}
\usepackage{wrapfig}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{siunitx}
\usepackage{physics}
%\usepackage{bm}
\usepackage{ctex}
\usepackage{mathtools}
\usepackage{unicode-math}  % 使用 unicode-math 替代 amsmath
\usepackage{tcolorbox}
\tcbuselibrary{breakable}
\usepackage{xcolor}  % 放在 hyperref 之前
%\usepackage{parbox}
\usepackage[version=4]{mhchem}
\usepackage{mathrsfs}
\usepackage{empheq}
\usepackage{fancybox}
\usepackage{arcs}
\DeclareFontFamily{OMX}{yhex}{}
\DeclareFontShape{OMX}{yhex}{m}{n}{<->yhcmex10}{}
\DeclareSymbolFont{yhlargesymbols}{OMX}{yhex}{m}{n}
\DeclareMathAccent{\wideparen}{\mathord}{yhlargesymbols}{"F3}


% 定义自定义框体样式
\newtcolorbox{examplebox}{
	colback = white,     % 背景色
	colframe = black,    % 边框色
	sharp corners,       % 直角边框
	boxrule = 0.8pt,     % 边框粗细
	% enhanced,            % 启用高级特性
	breakable,           % 允许跨页
	before skip = 10pt,  % 上方间距
	after skip = 10pt,   % 下方间距
	left = 5pt,          % 左侧内边距
	right = 5pt,         % 右侧内边距
	top = 5pt,           % 顶部内边距
	bottom = 5pt,        % 底部内边距
	fontupper = \normalsize, % 内容字号
	% title = 例题与练习,   % 框体标题
	coltitle = black,    % 标题颜色
	attach title to upper, % 标题与内容连接
}

% 定义数学符号
\newcommand{\ee}{\mathrm{e}}
\newcommand{\ii}{\mathrm{i}}
\newcommand{\intinf}{\int_{-\infty}^{\infty}}

% 设置页面布局（增加 headheight 以满足 fancyhdr 要求）
\geometry{a4paper, left=25mm, right=25mm, top=25mm, bottom=25mm, headheight=14pt}

% 设置页眉页脚
\pagestyle{fancy}
\fancyhf{}
\fancyhead[RE]{\leftmark}
\fancyhead[LO]{\rightmark}
\fancyfoot[C]{\thepage}

% 定义定理环境
\newtheorem{theorem}{Theorem}[chapter]
\newtheorem{definition}[theorem]{Definition}
\newtheorem{example}[theorem]{Example}

% 设置标题格式
\title{\Huge\bfseries 量子化学}
\author{\Large Ira N. Levine}
\date{\large 第七版}


\begin{document}
	
	\maketitle
	
	\frontmatter
	\tableofcontents
	
	\mainmatter

	% ===== CHAPTERS =====
	
	\input{chapters/01.tex}
	
	\input{chapters/02.tex}
	
	\input{chapters/03.tex}
	
	\input{chapters/04.tex}
	
	\input{chapters/05.tex}
	
	\input{chapters/06.tex}
	
	\input{chapters/07.tex}
	
	\input{chapters/08.tex}
	
	\input{chapters/09.tex}
	
	\input{chapters/10.tex}
	
	\input{chapters/11.tex}
	
	\input{chapters/12.tex}
	
	\input{chapters/13.tex}
	
	\input{chapters/14.tex}
	
	\input{chapters/15.tex}
	
	\input{chapters/16.tex}
	
	\input{chapters/17.tex}
	
	% ===== APPENDIX =====

	\appendix
	
	\input{chapters/appendix.tex}
	
	% ===== BIBLIOGRAPHY =====
	
	\input{chapters/bibliography.tex}
	
	% ===== ANSWERS TO SELECTED PROBLEMS =====
	
	\input{chapters/answers.tex}
	
	% ===== INDEX =====
	
	\input{chapters/index.tex}
	
\end{document}