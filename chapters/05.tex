...
\label{sec:5.1 Simultaneous Specification of Several Properties}
    ...
    \begin{examplebox}
        ...
        \begin{equation*}
            \left[\hat{x},\hat{p_x}\right] = \left[\hat{x},\frac{\hbar}{\ii }\frac{\partial}{\partial x}\right] = \frac{\hbar}{\ii }\left[\hat{x},\frac{\partial}{\partial x}\right] = -\frac{\hbar}{\ii }\left[\frac{\partial}{\partial x},\hat{x}\right] = -\frac{\hbar}{\ii }
        \end{equation*}
        \begin{equation}
            \left[\hat{x},\hat{p_x}\right]=\ii \hbar
            \label{eq:5.6}
        \end{equation}
        (b)由式(\ref{eq:5.5})和(\ref{eq:5.6})，有
        \begin{equation*}
            \left[\hat{x},\hat{p_x^2}\right] = \left[\hat{x},\hat{p_x}\right]\hat{p_x} + \hat{p_x}\left[\hat{x},\hat{p_x}\right] = \ii \hbar\cdot\frac{\hbar}{\ii }\frac{\partial}{\partial x} + \frac{\hbar}{\ii }\frac{\partial}{\partial x} \cdot \ii \hbar
        \end{equation*}
        \begin{equation}
        ...
        \end{equation*}
        \begin{equation}
            \left[\hat{x},\hat{H}\right] = \frac{\hbar^2}{m}\frac{\partial}{\partial x} = \frac{\ii \hbar}{m}\hat{p_x}
            \label{eq:5.8}
        \end{equation}
        \\
        \textbf{练习：}求证：对于单粒子三维系统，有
        \begin{equation}
            \left[\hat{p_x^2},\hat{H}\right] = -\ii \hbar \:\partial V\left(x,y,z\right)/\partial x
            \label{eq:5.9}
        \end{equation}
    \end{examplebox}
    ...
    \begin{equation}
        \left(\Delta A\right)^2 \equiv \sigma^2\left(A\right) = \langle \left(A - \langle A \rangle\right)^2 \rangle = \int \Psi^{\ast}\left(\hat{A}-\langle A \rangle\right)^2\Psi\dd \tau
        \label{eq:5.10}
    \end{equation}
    ...
    \begin{equation}
        \sigma\left(A\right)\sigma\left(B\right) = \Delta A \Delta B \ge \frac{1}{2}\left|\int \Psi^{\ast}\left[\hat{A},\hat{B}\right]\Psi\dd \tau\right|
        \label{eq:5.12}
    \end{equation}
    ...
    \begin{equation*}
        \Delta x \Delta p_x \ge \frac{1}{2}\left|\int \Psi^{\ast}\left[\hat{x},\hat{p_x}\right]\Psi\dd \tau\right| = \frac{1}{2}\left|\int \Psi^{\ast}\ii \hbar\Psi\dd \tau\right| = \frac{1}{2}\hbar\left|\ii \right|\left|\int \Psi^{\ast}\Psi\dd \tau\right|
    \end{equation*}
    \begin{equation}
    ...
    \begin{equation}
        \varepsilon\left(A\right)\eta\left(B\right) \ge \frac{1}{2}\left|\int \Psi^{\ast}\left[\hat{A},\hat{B}\right]\Psi\dd \tau\right|
        \label{eq:5.14}
    \end{equation}
    ...
    \begin{equation*}
        \varepsilon\left(A\right)\eta\left(B\right) + \varepsilon\left(A\right)\sigma\left(B\right) + \sigma\left(A\right)\eta\left(B\right) \ge \frac{1}{2}\left|\int \Psi^{\ast}\left[\hat{A},\hat{B}\right]\Psi\dd \tau\right|
    \end{equation*}
    ...
    \begin{equation*}
        \varepsilon\left(A\right)\varepsilon\left(B\right) \ge \frac{1}{2}\left|\int \Psi^{\ast}\left[\hat{A},\hat{B}\right]\Psi\dd \tau\right|
    \end{equation*}
...
\label{sec:5.2 Vectors}
    ...
    \begin{figure}[ht]
        ...
        \caption{单位矢量$\ii $、$\mathrm{j}$和$\mathrm{k}$，$\mathrm{A}$的三个分量}
        \label{fig:5.2}
    \end{figure}
    ...
    \begin{equation}
        ...
        \label{eq:5.29}
    \end{equation}
    则由式(\ref{eq:3.23})，线动量的矢量算符为$\hat{\mathbf{p}}=-\ii \hbar\nabla$。
    ...
    \begin{equation}
        \frac{\dd \mathbf{A}}{\dd t} = \mathbf{i}\frac{\dd A_x}{\dd t} + \mathbf{j}\frac{\dd A_y}{\dd t} + \mathbf{k}\frac{\dd A_z}{\dd t}
        \label{eq:5.32}
    \end{equation}

    矢量符号是表示函数变量的一种便捷方法。双粒子系统波函数可以写作$\psi\left(x_1,y_1,z_1,x_2,y_2,z_2\right)$，如果矢量$\mathbf{r}_1$从原点指向粒子1，那么$\mathbf{r}_1$就具有$x,y,z$三个方向的分量，指定$\mathbf{r}_1$等同于指定三个坐标$x,y,z$。$\mathbf{r}_2$也是如此。因此，我们可以将波函数写成$\psi\left(\mathbf{r}_1,\mathbf{r}_2\right)$。矢量符号也可以用在积分中，例如式(\ref{eq:3.57})对全空间的积分经常写成$\int \cdots \int\left|\Psi\left(\mathbf{r}_1,\cdots,\mathbf{r}_n\right)\right|^2\dd \mathbf{r}_1\cdots\dd \mathbf{r}_n$。
...
\subsection*{$n$维空间的矢量}
    ...
    函数的变量通常使用 $n$ 维矢量符号表示。例如，在双粒子系统中，与$\psi\left(x_1,y_1,z_1,x_2,y_2,z_2\right)$不同，我们可以定义一个六维矢量$\mathbf{q}$，其分量满足$q_1=x_1, q_2=y_1, q_3=z_1, q_4=x_2, q_5=y_2, q_6=z_2$，且将波函数写成 $\psi\left(\mathbf{q}\right)$。对于有$n$个粒子的系统，我们可以定义一个有$3n$个分量的矢量$\mathbf{q}$，将波函数写成$\psi\left(\mathbf{q}\right)$，使用$\int\left|\psi\left(\mathbf{q}\right)\right|^2\dd \mathbf{q}$来进行归一化处理。
...
\label{sec:5.3 Angular Momentum of a One-Particle System}
    ...
    \begin{equation}
        \mathbf{v} \equiv \frac{\dd \mathbf{r}}{\dd t} = \mathbf{i}\frac{\dd x}{\dd t} + \mathbf{j}\frac{\dd y}{\dd t} + \mathbf{k}\frac{\dd z}{\dd t}
        \label{eq:5.34}
    \end{equation}
    \begin{equation*}
        v_x = \dd x/\dd t, \quad v_y = \dd y/\dd t, \quad v_z = \dd z/\dd t
    \end{equation*}

    我们定义粒子的\textbf{线动量}（linear momentum）的矢量$\mathbf{p}$为
    \begin{equation}
        \boxed{
    ...
    作用在粒子上的\textit{力矩}（torque）$\tau$定义为力$\mathbf{F}$和位置矢量$\mathbf{r}$的叉积：$\tau \equiv \mathbf{r} \times \mathbf{F}$。可以证明：$\tau = \dd \mathbf{L} / \dd t$。当没有力矩作用于粒子时，其角动量的变化率为零；也就是说，其角动量是恒定的（或守恒的）。对于绕太阳运行的行星来说，引力是沿径向分布的。由于两个平行矢量的交积为零，因此行星上没有力矩，其角动量是守恒的。
...
\subsection*{单粒子轨道角动量算符}
    ...
    \begin{equation}
        \hat{L}_x = -\ii \hbar\left(y\frac{\partial}{\partial z} - z\frac{\partial}{\partial y}\right)
        \label{eq:5.40}
    \end{equation}
    \begin{equation}
        \hat{L}_y = -\ii \hbar\left(z\frac{\partial}{\partial x} - x\frac{\partial}{\partial z}\right)
        \label{eq:5.41}
    \end{equation}
    \begin{equation}
        \hat{L}_z = -\ii \hbar\left(x\frac{\partial}{\partial y} - y\frac{\partial}{\partial x}\right)
        \label{eq:5.42}
    \end{equation}
    ...
    \begin{equation*}
        \hat{L}_yf\left(x,y,z\right) = -\ii \hbar\left(z\frac{\partial f}{\partial x} - x\frac{\partial f}{\partial z}\right)
    \end{equation*}
    ...
    同样地，
    \begin{equation*}
        \hat{L}_xf = -\ii \hbar\left(y\frac{\partial f}{\partial z} - z\frac{\partial f}{\partial y}\right)
    \end{equation*}
    ...
    \end{equation*}
    \begin{equation}
        \left[\hat{L}_x, \hat{L}_y\right] = \ii \hbar\hat{L}_z
        \label{eq:5.46}
    \end{equation}
    其中我们用到了类似以下的关系：
    \begin{equation}
    ...
    \begin{equation}
        \left[\hat{L}_y, \hat{L}_z\right] = \ii \hbar\hat{L}_x, \quad \left[\hat{L}_z, \hat{L}_x\right] = \ii \hbar\hat{L}_y
        \label{eq:5.48}
    \end{equation}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            & = -\ii \hbar\hat{L}_z\hat{L}_y - \ii \hbar\hat{L}_y\hat{L}_z + \ii \hbar\hat{L}_y\hat{L}_z + \ii \hbar\hat{L}_z\hat{L}_y \\
        \end{aligned} 
    \end{equation*}
    \begin{equation}
        \boxed{
    ...
    \begin{equation*}
        \begin{aligned}
            \hat{L}_x & = -\ii \hbar\left[r\sin\theta\sin\phi\left(\cos\theta\frac{\partial}{\partial r}\right) - \frac{\sin\theta}{r}\frac{\partial }{\partial\theta}\right.\\
    ...
    \end{equation*}
    \begin{equation}
        \hat{L}_x = \ii \hbar\left(\sin\phi\frac{\partial}{\partial \theta} - \cos\theta\cos\phi\frac{\partial}{\partial \phi}\right)
        \label{eq:5.65}
    \end{equation}
    同样，我们有
    \begin{equation}
        \hat{L}_y = -\ii \hbar\left(\cos\phi\frac{\partial}{\partial \theta} - \cot\theta\sin\phi\frac{\partial}{\partial \phi}\right)
        \label{eq:5.66}
    \end{equation}
    \begin{equation}
        \hat{L}_z = -\ii \hbar\frac{\partial}{\partial \phi}
        \label{eq:5.67}
    \end{equation}
...
\subsection*{单粒子轨道角动量的本征函数和本征值}
    ...
    使用算符$\hat{L}_z$，我们有
    \begin{equation}
        -\ii \hbar\frac{\partial}{\partial \phi}Y\left(\theta,\phi\right) = bY\left(\theta,\phi\right)
        \label{eq:5.71}
    \end{equation}
    ...
    则方程(\ref{eq:5.71})变为
    \begin{equation*}
        -\ii \hbar\frac{\partial}{\partial \phi}\left[S\left(\theta\right)T\left(\phi\right)\right] = bS\left(\theta\right)T\left(\phi\right)
    \end{equation*}
    \begin{equation*}
        -\ii \hbar S\left(\theta\right)\frac{\dd T\left(\phi\right)}{\dd \phi} = bS\left(\theta\right)T\left(\phi\right)
    \end{equation*}
    \begin{equation*}
        \frac{\dd T\left(\phi\right)}{T\left(\phi\right)} = \frac{\ii b}{\hbar}\dd \phi
    \end{equation*}
    \begin{equation}
        T\left(\phi\right) = A e^{\ii b\phi/\hbar}
        \label{eq:5.73}
    \end{equation}
    其中$A$是任意常数。
    ...
    \end{equation*}
    \begin{equation*}
        A e^{\ii b\phi/\hbar}\ee ^{\ii b2\pi/\hbar} = A e^{\ii b\phi/\hbar}
    \end{equation*}
    \begin{equation}
        \ee ^{\ii b2\pi/\hbar} = 1
        \label{eq:5.74}
    ...
    为了满足$\ee ^{\ii \alpha} = \cos\alpha + \ii \sin\alpha = 1$，我们需要$\alpha = 2\pi m$，其中
    \begin{equation*}
    ...
    式(\ref{eq:5.73})变为
    \begin{equation}
        T\left(\phi\right) = A e^{\ii m\phi}, \quad m = 0, \pm 1, \pm 2, \ldots
        \label{eq:5.76}
    \end{equation}
    ...
    \begin{equation}
        ...
        \label{eq:5.78}
    \end{equation}
    (\ref{eq:5.78})是一个无限小区域空间的体积，其球面坐标位于$r$到$r+\dd r$、$\theta$到$\theta+\dd \theta$和$\phi$到$\phi+\dd \phi$之间。因此，球坐标下$F$的归一化条件为
    \begin{equation}
        \int_{0}^{\infty}\left[\int_{0}^{\pi}\left[\int_{0}^{2\pi}\left|F\left(r,\theta,\phi\right)\right|^2 \dd \phi\right]\sin\theta \dd \theta\right]r^2\dd r = 1
        \label{eq:5.79}
    \end{equation}
    如果$F$恰好可以写成
    \begin{equation*}
    ...
    \begin{equation*}
        \int_{0}^{\infty}\left|R\left(r\right)\right|^2 r^2 \dd r \int_{0}^{\pi}\left|S\left(\theta\right)\right|^2 \sin\theta \: \dd \theta \int_{0}^{2\pi}\left|T\left(\phi\right)\right|^2 \dd \phi = 1
    \end{equation*}
    方便的做法是将$F$的每个因子都作归一化处理：
    \begin{equation}
        \int_{0}^{\infty}\left|R\left(r\right)\right|^2 r^2 \dd r = 1, \quad \int_{0}^{\pi}\left|S\left(\theta\right)\right|^2 \sin\theta \: \dd \theta = 1, \quad \int_{0}^{2\pi}\left|T\left(\phi\right)\right|^2 \dd \phi = 1
        \label{eq:5.80}
    \end{equation}
    因此，
    \begin{equation*}
        \int_{0}^{2\pi}\left(A\ee ^{\ii m\phi}\right)^{\ast}\left(A\ee ^{\ii m\phi}\right) \dd \phi = 1 = \left|A\right|^2\int_{0}^{2\pi} \dd \phi
    \end{equation*}
    \begin{equation*}
        \left|A\right| = \left(2\pi\right)^{-1/2}
    \end{equation*}
    \begin{equation}
        T\left(\phi\right) = \frac{1}{\sqrt{2\pi}}e^{\ii m\phi}, \quad m = 0, \pm 1, \pm 2, \ldots
        \label{eq:5.81}
    \end{equation}

    我们现在尝试使用式(\ref{eq:5.70})来求解$\hat{L}^2$的本征值$c$。对$\hat{L}^2$使用式(\ref{eq:5.68})、对$Y$使用式(\ref{eq:5.72})，由式(\ref{eq:5.81})，我们有
    \begin{equation*}
        -\hbar^2\left(\frac{\partial^2}{\partial\theta^2} + \cot\theta\frac{\partial}{\partial\theta} + \frac{1}{\sin^2\theta}\frac{\partial^2}{\partial\phi^2}\right)\left(S\left(\theta\right)\frac{1}{\sqrt{2\pi}}e^{\ii m\phi}\right) = cS\left(\theta\right)\frac{1}{\sqrt{2\pi}}e^{\ii m\phi}
    \end{equation*}
    \begin{equation}
        \frac{\dd ^2S}{\dd \theta^2}+\cot\theta\frac{\dd S}{\dd \theta} - \frac{m^2}{\sin^2\theta}S = -\frac{c}{\hbar^2}S
        \label{eq:5.82}
    \end{equation}
    \begin{quote}
        \small
        ...
        由链式法则，我们有
        \begin{equation}
            \frac{\dd S}{\dd \theta} = \frac{\dd G}{\dd w}\frac{\dd w}{\dd \theta} = -\sin\theta\frac{\dd G}{\dd w} = -\left(1-w^2\right)^{1/2}\frac{\dd G}{\dd w}
            \label{eq:5.85}
        \end{equation}
        同样地，我们得出（问题5.25）
        \begin{equation}
            \frac{\dd ^2S}{\dd \theta^2} = \left(1-w^2\right)\frac{\dd ^2G}{\dd w^2} - w\frac{\dd G}{\dd w}
            \label{eq:5.86}
        \end{equation}
        由式(\ref{eq:5.86})、(\ref{eq:5.85})和$\cot\theta = \cos\theta/\sin\theta = w/\left(1-w^2\right)^{1/2}$，我们可以将(\ref{eq:5.82})重写为
        \begin{equation}
            \left(1-w^2\right)\frac{\dd ^2G}{\dd w^2} - 2w\frac{\dd G}{\dd w}+\left[\frac{c}{\hbar^2}-\frac{m^2}{1-w^2}\right]G\left(w\right) = 0
            \label{eq:5.87}
        \end{equation}
    ...
    \begin{equation}
        \boxed{
            Y^m_l\left(\theta,\phi\right) = S_{l,m}\left(\theta\right)T\left(\phi\right) = \frac{1}{\sqrt{2\pi}}S_{l,m}\left(\theta\right)e^{\ii m\phi}
        }
        \label{eq:5.99}
    \end{equation}
    ...
    \begin{examplebox}
        ...
        \begin{equation*}
            \int_{0}^{\pi}\left|a_0\right|^2 \sin\theta \: \dd \theta = 1 = 2\left|a_0\right|^2
        \end{equation*}
        \begin{equation*}
        ...
        \begin{equation*}
            1 = \left|a_0^2\right|\int_{0}^{\pi}\sin^2\theta\sin\theta \: \dd \theta = \left|a_0^2\right|\int_{-1}^{1}\left(1-w^2\right) \: \dd w
        \end{equation*}
        \begin{equation*}
        ...
        \begin{equation}
            Y_1^1 = \left(3/8\pi\right)^{1/2}\sin\theta e^{\ii \phi}, \quad Y_1^{-1} = \left(3/8\pi\right)^{1/2}\sin\theta e^{-\ii \phi}
            \label{eq:5.103}
        \end{equation}
    ...
    \begin{equation}
        \Delta L_x\Delta L_y \ge \left|\frac{1}{2}\Psi^{\ast}\left[\hat{L}_x,\hat{L}_y\right]\Psi\dd \tau\right| = \frac{\hbar}{2}\left|\int \Psi^{\ast}\hat{L}_z\Psi\dd \tau\right|
        \label{eq:5.106}
    \end{equation}
...
\label{sec:5.4 The Ladder-Operator Method for Angular Momentum}
    ...
    \begin{equation}
        \left[\hat{M}_x,\hat{M}_y\right] = \ii \hbar\hat{M}_z, \quad \left[\hat{M}_y,\hat{M}_z\right] = \ii \hbar\hat{M}_x, \quad \left[\hat{M}_z,\hat{M}_x\right] = \ii \hbar\hat{M}_y
        \label{eq:5.107}
    \end{equation}
    我们定义算符$\hat{M}^2$为
    \begin{equation}
    ...
    \begin{equation}
        \hat{M}_+ \equiv \hat{M}_x + \ii \hat{M}_y
        \label{eq:5.110}
    \end{equation}
    \begin{equation}
        \hat{M}_- \equiv \hat{M}_x - \ii \hat{M}_y
    ...
    \begin{equation*}
        \begin{aligned}
            \hat{M}_+\hat{M}_- & = \left(\hat{M}_x+\ii \hat{M}_y\right)\left(\hat{M}_x-\ii \hat{M}_y\right) = \hat{M}_x\left(\hat{M}_x-\ii \hat{M}_y\right) + \ii \hat{M}_y\left(\hat{M}_x-\ii \hat{M}_y\right) \\
            & = \hat{M}_x^2 - \ii \hat{M}_x\hat{M}_y + \ii \hat{M}_y\hat{M}_x + \hat{M}_y^2 = \hat{M}^2 - \hat{M}_z^2 + \ii \left[\hat{M}_y,\hat{M}_x\right]
        \end{aligned}
    \end{equation*}
    \begin{equation}
    ...
    \begin{equation*}
        \left[\hat{M}_+,\hat{M}_z\right] = \left[\hat{M}_x+\ii \hat{M}_y,\hat{M}_z\right] = \left[\hat{M}_x,\hat{M}_z\right]+\ii \left[\hat{M}_y,\hat{M}_z\right] = -\ii \hbar\hat{M}_y - \hbar\hat{M}_x
    \end{equation*}
    \begin{equation*}
    ...
    \begin{equation*}
        \left[\hat{M}^2,\hat{M}_{\pm}\right] = \left[\hat{M}^2,\hat{M}_x\pm\ii \hat{M}_y\right] = \left[\hat{M}^2,\hat{M}_x\right] \pm \ii \left[\hat{M}^2,\hat{M}_y\right] = 0 \pm 0 = 0
    \end{equation*}
    也有
    \begin{equation*}
