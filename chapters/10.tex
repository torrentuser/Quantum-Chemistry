...
\label{sec:10.1 Electron Spin}
    ...
    \begin{equation}
        ...
        \label{eq:10.1}
    \end{equation}
    假设自旋角动量算符遵循轨道角动量算符相同的对易关系。与$\left[\hat{L}_x, \hat{L}_y\right] = \ii \hbar \hat{L}_z$，$\left[\hat{L}_y, \hat{L}_z\right] = \ii \hbar \hat{L}_x$，$\left[\hat{L}_z, \hat{L}_x\right] = \ii \hbar \hat{L}_y$[式(\ref{eq:5.46})和(\ref{eq:5.48})]，我们有
    \begin{equation}
        \left[\hat{S}_x, \hat{S}_y\right] = \ii \hbar \hat{S}_z, \quad
        \left[\hat{S}_y, \hat{S}_z\right] = \ii \hbar \hat{S}_x, \quad
        \left[\hat{S}_z, \hat{S}_x\right] = \ii \hbar \hat{S}_y.
        \label{eq:10.2}
    \end{equation}
    ...
    \begin{figure}[ht]
        ...
        \caption{
            ...
            \parbox{0.4\linewidth}{
                \noindent
                电子自旋矢量相对于 $z$ 轴的可能方向。在每种情况下，$\mathbf{S}$ 都位于以 $z$ 轴为轴的圆锥表面上。
            }
        }
        \label{fig:10.1}
    \end{figure}
    ...
    \begin{equation*}
        \intinf \intinf \intinf \left|\psi\left(x, y, z\right)\right|^2\: \dd x\: \dd y\: \dd z = 1
    \end{equation*}
    ...
    \begin{equation}
        \boxed{
            \sum_{m_s=-1/2}^{1/2} \intinf \intinf \intinf \left|\psi\left(x, y, z, m_s\right)\right|^2 \:\dd x \:\dd y \:\dd z = 1
        }
        \label{eq:10.13}
    \end{equation}
    符号
    \begin{equation*}
        \int \left|\psi\left(x, y, z, m_s\right)\right|^2 \:\dd \tau
    \end{equation*}
    表示对自旋变量求和，对空间变量全范围积分，如 (\ref{eq:10.13}) 所示。符号 $\int \:\dd v$ 表示对系统空间变量全范围的积分。
...
\label{sec:10.4 Th Helium Atom}
    ...
    \begin{equation*}
        \begin{aligned}
            \int \phi^* \hat{H} \phi \: \dd \tau =& \sum_{m_{s1}} \sum_{m_{s2}} \int \int f^* \left(r_1, r_2, r_{12}\right) \frac{1}{\sqrt{2}} \left[\alpha\left(1\right)\beta\left(2\right) - \beta\left(1\right)\alpha\left(2\right)\right]^* \\
            & \times \hat{H} f \left(r_1, r_2, r_{12}\right) \frac{1}{\sqrt{2}} \left[\alpha\left(1\right)\beta\left(2\right) - \beta\left(1\right)\alpha\left(2\right)\right] \: \dd v_1 \: \dd v_2
        \end{aligned}
    \end{equation*}
    因为$\hat{H}$对自旋波函数没有作用，所以变分积分变为
    \begin{equation*}
        \int \int f^* \hat{H} f \:\dd v_1 \: \dd v_2 \sum_{m_{s1}} \sum_{m_{s2}} \frac{1}{2} \left[\alpha\left(1\right)\beta\left(2\right) - \beta\left(1\right)\alpha\left(2\right)\right]^2
    \end{equation*}
    又因为自旋函数(\ref{eq:10.25})是归一化的，所以变分积分简化为$\int \int f^* \hat{H} f \:\dd v_1 \: \dd v_2$，这就是我们引入自旋之前的变分积分。
...
\label{sec:10.5 The Pauli Exclusion Principle}
    ...
    假设我们把锂原子中的电子间斥力看作是对哈密顿其余项的微扰。按照处理氦的相同步骤，无扰波函数是三个类氢函数的乘积。对于基态
    \begin{equation}
        \psi^{\left(0\right)} = 1s\left(1\right)1s\left(2\right)1s\left(3\right)
        \label{eq:10.31}
    \end{equation}
    零级（无扰）能量为[方程(\ref{eq:9.48})及(\ref{eq:9.50})之后的段落]
    \begin{equation*}
        E^{\left(0\right)} = -\left(\frac{1}{1^2} + \frac{1}{2^2} + \frac{1}{1^2}\right)\left(\frac{Z^2\ee ^2}{8\pi \varepsilon_0a_0}\right) = -27 \left(\frac{\ee ^2}{8\pi \varepsilon_0a_0}\right) = -27  \left(13.606 \: \mathrm{eV}\right) = -367.4 \: \mathrm{eV}
    \end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            E^{\left(1\right)} = \int \left|1s\left(1\right)\right|^2 \left|1s\left(2\right)\right|^2 \left|1s\left(3\right)\right|^2 \frac{\ee ^2}{4\pi\varepsilon_0 r_{12}} \:\dd v &+ \int \left|1s\left(1\right)\right|^2 \left|1s\left(2\right)\right|^2 \left|1s\left(3\right)\right|^2 \frac{\ee ^2}{4\pi\varepsilon_0 r_{23}} \:\dd v \\
            &+ \int \left|1s\left(1\right)\right|^2 \left|1s\left(2\right)\right|^2 \left|1s\left(3\right)\right|^2 \frac{\ee ^2}{4\pi\varepsilon_0 r_{13}} \:\dd v
        \end{aligned}
    \end{equation*}
    在这些定积分中，我们给虚拟积分变量贴标签的方式不会影响它们的值。如果我们互换第二个积分中变量的标号 1 和 3，它就会转换成第一个积分。因此，这两个积分相等。将第三个积分中的标号 2 和 3 互换后，它也等于第一个积分。因此
    \begin{equation*}
        E^{\left(1\right)} = 3 \int \int \left|1s\left(1\right)\right|^2 \left|1s\left(2\right)\right|^2 \frac{\ee ^2}{4\pi \varepsilon_0 r_{12}} \:\dd v_1\: \dd v_2 \int \left|1s\left(3\right)\right|^2 \:\dd v_3
    \end{equation*}
    对电子3积分的值为1（归一化）。电子 1 和电子 2 的积分是在氦的微扰处理中计算得到的，以及[式(\ref{eq:9.52})和(\ref{eq:9.53})]
    \begin{equation*}
        E^{\left(1\right)} = 3\left(\frac{5Z}{4}\right)\left(\frac{\ee ^2}{8\pi \varepsilon_0a_0}\right) = \frac{45}{4} \left(13.606 \: \mathrm{eV}\right) = 153.1 \: \mathrm{eV}
    \end{equation*}
    \begin{equation*}
        E^{\left(0\right)} + E^{\left(1\right)} = -214.3 \: \mathrm{eV}
    \end{equation*}

    因为我们可以把零级受扰波函数作为试探函数（回顾第 \ref{sec:9.4 Variational Treatments of the Ground State of Helium} 节开头的讨论），根据变分原理，$E^{\left(0\right)} + E^{\left(1\right)}$等于或大于真实基态能量。锂基态能量的实验值通过将第一、第二和第三电离能相加得到，即[C. E. Moore, “Ionization Potentials and Ionization Limits,” publication NSRDS-NBS 34 of the National Bureau of Standards (1970); available at \url{www.nist.gov/data/nsrds/NSRDSNBS34.pdf}]
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            \int \left[f\left(1\right)g\left(2\right)h\left(3\right)\right]^*&f\left(2\right)g\left(1\right)h\left(3\right) \:\dd \tau \\
            &= \int f^*\left(1\right)g\left(1\right)\:\dd \tau_1 \int g^*\left(2\right)f\left(2\right)\:\dd \tau_2 \int h^*\left(3\right)h\left(3\right)\:\dd \tau_3 = 0 \cdot 0 \cdot 1 = 0
        \end{aligned}
    \end{equation*}
    由于 $f$、$g$和$h$已归一化，因此涉及 (\ref{eq:10.36}) 项与其自身复共轭的乘积的积分等于 1。因此，
    \begin{equation*}
        1 = \int \left|10.36\right|^2 \:\dd \tau = \left|c_1\right|^2 \left(1+1+1+1+1+1\right)
    \end{equation*}
    \begin{equation*}
        c_1 = 1 / \sqrt{6}
    \end{equation*}
    ...
    虽说组态$1s^22p$与$1s^22s$具有相同的无扰能量$E^{\left(0\right)}$，但当我们在计算$E^{\left(1\right)}$以及更高级的校正时引入了电子斥力，就会发现 $1s^22s$ 组态的能量较低，原因与氦的情况相同。
...
\label{sec:10.7 Perturbation Treatment of the Lithium Ground State}
    ...
    让我们对锂原子的基态进行微扰处理。定义$\ee ' \equiv \ee / 4\pi \varepsilon_0$，我们有
    \begin{equation*}
        \hat{H}^0 = \frac{-\hbar^2}{2m_{\ee }}\nabla_1^2 - \frac{\hbar^2}{2m_{\ee }}\nabla_2^2 - \frac{\hbar^2}{2m_{\ee }}\nabla_3^2 - \frac{Z\ee '^2}{r_{1}} - \frac{Z\ee '^2}{r_{2}} - \frac{Z\ee '^2}{r_{3}}, \quad \hat{H}' = \frac{\ee '^2}{r_{12}} + \frac{\ee '^2}{r_{13}} + \frac{\ee '^2}{r_{23}}
    \end{equation*}
    ...
    \begin{equation}
        E^{\left(0\right)} = -\left(\frac{1}{1^2} + \frac{1}{1^2} + \frac{1}{2^2}\right) \left(\frac{Z^2\ee '^2}{2a_0}\right) = -\frac{81}{4}\left(13.606 \: \mathrm{eV} \right) = -275.5 \: \mathrm{eV}
        \label{eq:10.49}
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            E^{\left(1\right)} = 2\int \int 1s^2\left(1\right)2s^2\left(2\right) \frac{\ee '^2}{r_{12}} \: \dd v_1 \: \dd v_2 &+ \int \int 1s^2\left(1\right)1s^2\left(2\right) \frac{\ee '^2}{r_{12}} \: \dd v_1 \: \dd v_2 \\
            &- \int \int 1s\left(1\right)2s\left(2\right)1s\left(2\right)2s\left(1\right) \frac{\ee '^2}{r_{12}} \: \dd v_1 \: \dd v_2
        \end{aligned}
        \label{eq:10.50}
    \end{equation}
    这些积分是库仑积分和交换积分：
    ...
    \begin{equation*}
        J_{1s1s} = \frac{5}{8} \frac{Z\ee '^2}{a_0}, \quad J_{1s2s} = \frac{17}{81} \frac{Z\ee '^2}{a_0}, \quad K_{1s2s} = \frac{16}{729} \frac{Z\ee '^2}{a_0}
    \end{equation*}
    \begin{equation*}
        E^{\left(1\right)} = \frac{5965}{972}\left(\frac{\ee '^2}{2a_0}\right) = 83.5 \: \mathrm{eV}
    \end{equation*}
...
\label{sec:10.8 Variation Treatments of the Lithium Ground State}
    ...
    \begin{equation}
        f \equiv \frac{1}{\pi^{1/2}}\left(\frac{b_1}{a_0}\right)^{3/2} \ee ^{-b_1r/a_0}
        \label{eq:10.52}
    \end{equation}
    其中，$b_1$ 是一个变分参数，代表 $1s$ 电子的有效核电荷。不使用表\ref{tab:6.2}中 $Z=3$ 的 $2s$ 函数，我们令
    \begin{equation}
        g = \frac{1}{4\left(2\pi\right)^{1/2}} \left( \frac{b_2}{a_0} \right)^{3/2} \left( 2 - \frac{b_2 r}{a_0} \right) \ee ^{-b_2 r/2a_0}
        \label{eq:10.53}
    \end{equation}
    则我们的试探变分函数为
    \begin{equation}
...
\label{sec:10.9 Spin Magnetic Moment}
    ...
    回忆：电子的轨道角动量$\mathbf{L}$有一磁矩$-\left(\ee /2m_{\ee }\right)\mathbf{L}$与之相关联[式(\ref{eq:6.128})]，其中$-\ee $是电子的电荷。我们很自然地假设也有磁矩$\mathbf{m}_S$与电子的自旋$\mathbf{S}$相关联。猜测$\mathbf{m}_S$为$-\left(\ee /2m_{\ee }\right)$乘以$\mathbf{S}$。然而，自旋是一个相对论现象，我们不能希望$\mathbf{m}_S$与$\mathbf{S}$的关系和$\mathbf{L}$与$\mathbf{S}$的关系完全一致。事实上，狄拉克对电子的相对论处理得出了以下结果（使用SI单位制）
    \begin{equation}
        \mathbf{m}_S = -g_{\ee } \frac{\ee }{2m_{\ee }} \mathbf{S}
        \label{eq:10.55}
    \end{equation}
    对于\textit{电子的$g$因子$g_{\ee }$}，狄拉克得出$g_{\ee } = 2$。在狄拉克的研究之后，理论和实验研究表明$g_{\ee }$实际上略大于2，$g_{\ee } = 2\left(1 + \alpha/2\pi + \ldots\right) = 2.0023$，其中的省略表示含$\alpha$更高次幂的项，其中\textit{精细结构常数}（fine-structure constant）$\alpha$为
    \begin{equation}
        \alpha \equiv \frac{\ee ^2}{4\pi\varepsilon_0\hbar c} = 0.00729735257
        \label{eq:10.56}
    \end{equation}
    对$g_{\ee }$的测量结果异常精确[D. Hanneke et al., \textit{Phys. Rev. A}, \textbf{83}, 052122 (2011); available at \url{arxiv.org/abs/1009.4831}]：$g_{\ee } = 2.002319304361$。[有些工作者倾向于省略 (\ref{eq:10.55}) 中的负号，而采用 $g_{\ee }$ 为负数的惯例；见\url{www.stanford.edu/group/Zarelab/publinks/zarepub642.pdf}。]

    电子自旋磁矩的大小为（采用SI单位制）
    \begin{equation}
        \left|\mathbf{m}_{S}\right| = g_{\ee } \frac{\ee }{2m_{\ee }} \left|\mathbf{S}\right| = g_{\ee } \sqrt{\frac{3}{4}} \frac{\ee \hbar}{2m_{\ee }}
        \label{eq:10.57}
    \end{equation}
...
\subsection*{核磁共振（NMR）谱}
    ...
    \begin{equation}
        \mathbf{m}_I = g_N\left(\ee / 2m_p\right)\mathbf{I} \equiv \gamma \mathbf{I}
        \label{eq:10.58}
    \end{equation}
    其中$m_p$是质子质量，\textit{核的$g$因子$g_N$}的值与核有关。$\gamma$称为\textbf{磁旋比}（magnetogyric ratio），定义为$\gamma \equiv \mathbf{m}_{I}/\mathbf{I} = g_{N}\ee /2m_{p}$。某些核的$I$、$g_N$和$\gamma$值如下表所示：
    \begin{table}[htbp]
        \centering
    ...
    \end{equation*}
    \begin{equation}
        \nu = \left(\left|\gamma\right|/2\pi\right)B = \left|g_N\right| \left(\ee /4\pi m_p\right)B
        \label{eq:10.60}
    \end{equation}
...
\label{sec:10.10 Ladder Operators for Electron Spin}
    ...
    \begin{equation}
        \hat{S}_+ = \hat{S}_x + \ii \hat{S}_y, \quad \hat{S}_- = \hat{S}_x - \ii \hat{S}_y
        \label{eq:10.63}
    \end{equation}
    ...
    自旋函数$\alpha$和$\beta$是$\hat{S}_z$的本征函数，其本征值分别为$+\frac{1}{2}\hbar$和$-\frac{1}{2}\hbar$。由于$\hat{S}_+$是升算符，则函数$\hat{S}_+\beta$是$\hat{S}_z$的本征函数，本征值为$+\frac{1}{2}\hbar$。具有该本征值的$\hat{S}_z$最一般的本征函数为任意常数乘以$\alpha$。因此，
    \begin{equation}
    ...
    \end{equation*}
    \begin{equation*}
        \left|c\right|^2 = \sum \left(\hat{S}_+\beta\right) ^* \left(\hat{S}_+\beta\right) = \sum \left(\hat{S}_+\beta\right)^* \left(\hat{S}_x + \ii \hat{S}_y\right)\beta
    \end{equation*}
    \begin{equation}
        \left|c\right|^2 = \sum \left(\hat{S}_+\beta\right)^* \hat{S}_x\beta + \ii \sum \left(\hat{S}_+\beta\right)^* \hat{S}_y\beta
        \label{eq:10.67}
    \end{equation}
    我们现在使用算符$\hat{S}_x$和$\hat{S}_y$的厄米性。对于作用在任意连续变量$x$上的算符$\hat{A}$，厄米性为
    \begin{equation*}
        \intinf f^*\left(x\right)\hat{A}g\left(x\right) \: \dd x = \intinf g\left(x\right) \left[\hat{A}f\left(x\right)\right]^* \: \dd x
    \end{equation*}
    ...
    \begin{equation*}
        c ^* c = \sum \beta (\hat{S}_x\hat{S}_+ \beta) ^* + \ii \sum \beta (\hat{S}_y\hat{S}_+ \beta)^*
    \end{equation*}
    取该方程的复共轭，使用(\ref{eq:10.63})和(\ref{eq:10.65})，我们得到
    \begin{equation*}
        \begin{aligned}
            cc^* &= \sum \beta ^* \hat{S}_x\hat{S}_+ \beta - \ii \sum \beta ^* \hat{S}_y\hat{S}_+ \beta \\
            |c|^2 &= \sum \beta ^* \left(\hat{S}_x - \ii \hat{S}_y  \right)\hat{S}_+\beta = \sum \beta ^* \hat{S}_-\hat{S}_+\beta \\
    ...
    同样地，有
    \begin{equation}
        \hat{S}_x \alpha = \frac{1}{2}\hbar \beta, \quad \hat{S}_y \alpha = \frac{1}{2}\ii \hbar \beta
        \label{eq:10.73}
    \end{equation}
